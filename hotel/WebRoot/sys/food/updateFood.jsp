...
<div id="MainArea">
	<!-- 表单内容 -->
	<form action="${pageContext.request.contextPath}/servlet/DetalServlet?method=update" method="post" ><!-- enctype="multipart/form-data" -->
		<!-- 本段标题（分段标题） -->
                    ...
                    <tr>
							...
							<td>
	                            <select name="cid" style="width:80px">	                            	
		                           <c:forEach var="foodType"  items="${requestScope.foodType}">		                        		
			                           	<option value="${foodType.id }" name="foodType_id">${foodType.typeName }</option>			          
		                           </c:forEach>				   					
	                            </select>
                             </td>			   				                
						</tr>
						<tr>
							...
							<td>
								...
								<input type="hidden" name="foodId" class="InputStyle" value="${requestScope.foodId}"/>
							</td>
						</tr>
						<tr>
							<td>价格</td>
